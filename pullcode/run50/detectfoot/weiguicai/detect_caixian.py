...
if __name__ == "__main__":
    ...
    cap = cv2.VideoCapture("ysyaxian.mp4")
    
    video_fps = cap.get(cv2.CAP_PROP_FPS)
    video_width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    video_height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))

    out_video = cv2.VideoWriter('./ysyaxian_out.mp4', cv2.VideoWriter_fourcc(*'mp4v'), video_fps, (video_width, video_height))
    ...
    while True:
        ...
        if k >= 50:
            cv2.putText(inp_frame, "2 runway planting line", (50, 750), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), thickness=6)
